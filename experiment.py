...
if __name__ == '__main__':
    ...
    samples, sample_rate = librosa.load('000204.wav', sr=22050)
    spec = librosa.stft(y=samples)
    mel = librosa.filters.mel(sample_rate, n_fft=2048, n_mels=128)
    ampl = np.dot(mel, np.abs(spec) ** 2)
    phase = np.angle(spec)
    phase_sin = np.dot(mel, (np.sin(phase)))
    phase_cos = np.dot(mel, (np.cos(phase)))
    phase_sin_shifted = np.dot(mel, (np.sin(phase) + 1) * 100)
    phase_cos_shifted = np.dot(mel, (np.cos(phase) + 1) * 100)
    amp = librosa.feature.inverse.mel_to_stft(ampl, power=2)
    sin = librosa.feature.inverse.mel_to_stft((phase_sin_shifted / 100) - 1, power=1)
    cos = librosa.feature.inverse.mel_to_stft((phase_cos_shifted / 100) - 1, power=1)
    S = amp * cos + amp * sin * 1j
    audio = librosa.istft(S)
    wavfile.write('outwav.wav', 22050, audio)
