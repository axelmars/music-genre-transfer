...
from keras.layers import Layer, Input, Reshape, Lambda, Flatten, Concatenate, Embedding, GaussianNoise, Permute, GRU, Softmax, Attention
...
class ConverterA:
    ...
    def build(cls, img_shape, n_imgs, n_identities,
        ...
        return ConverterA(config, pose_encoder, identity_embedding, identity_modulation, generator)
    ...
    def load(cls, model_dir, include_encoders=False):
        ...
        if not include_encoders:
            return ConverterA(config, pose_encoder, identity_embedding, identity_modulation, generator, model, None, epoch)

        identity_encoder = load_model(os.path.join(model_dir, 'identity_encoder.h5py'))

        return ConverterA(config, pose_encoder, identity_embedding, identity_modulation, generator, identity_encoder)
    ...
    def train(self, imgs, identities, batch_size, n_epochs, model_dir, tensorboard_dir):
        ...
        self.model.compile(
            optimizer=self.opt,
            loss=ConverterA.get_custom_loss(self.config)
        )
    ...
    def __l1_l2_and_perceptual_loss_multiscale(cls, y_true, y_pred, vgg, config):
        return 32 * tf.keras.losses.MeanSquaredError()(y_true, y_pred) + 0.01 * cls.__perceptual_loss_multiscale(y_true, y_pred, vgg, config)
    ...
    def __build_pose_encoder(cls, img_shape, pose_dim, pose_std, pose_decay):
        ...
        pose_code = Reshape(target_shape=(-1, 64))(x)
        # x = Flatten()(x)
        #
        # for i in range(2):
        #     x = Dense(units=256)(x)
...
class RestrictedSelfAttention(Layer):
    def __init__(self, units, k, **kwargs):
        super().__init__(**kwargs)
        self.units = units
        self.k = k
        self.Wq = Dense(self.units)
        self.Wk = Dense(self.units)

    def call(self, inputs, **kwargs):

        wk = self.Wk(inputs)

        paddings = tf.constant([[0, 0, ], [self.k, self.k], [0, 0]])
        pk = tf.pad(wk, paddings)
        pv = tf.pad(inputs, paddings)
        ...
        vals = vals[:, self.k:-self.k, :, :]

        # -- missing code --

        query = self.Wq(inputs)

        dot_product = tf.einsum('lij,likj->lik', query, keys) / np.sqrt(self.units)

        atten_weights = Softmax(name="atten_weights", axis=1)(dot_product)

        val_out = tf.einsum('lij,lijk->lik', atten_weights, vals)

        return inputs + val_out
